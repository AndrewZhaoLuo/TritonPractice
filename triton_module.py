...
class TransformerGatedLinearLayerFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output) -> Any:
        (
            ...
            weight_grad,
            bias_grad,
        ) = TransformerGatedLinearLayerFunction.backward_reference(ctx, grad_output)
        input_tensor, weight_tensor, bias_tensor = ctx.saved_tensors
...
class OptimizedTransformerGatedLinearLayer(nn.Module):
    ...
    def from_torch(torch_module: BaseTransformerGatedLinearLayer):
        return OptimizedTransformerGatedLinearLayer(
            torch_module.dimension_in,
            torch_module.projection_factor,
            torch_module.linear.weight.dtype,
            weight_init=torch_module.linear.weight,
            bias_init=torch_module.linear.bias,
        )
...
if __name__ == "__main__":
    ...
    for i in range(repeats):
        ...
        print("Grad weight: ")
        print_is_close(
            torch_layer.linear.weight.grad,
